...
def crearJugadores():
    ...
    cantidadJugadores = int(input("Cuantos jugadores formaran parte de la partida? \nEl rango debe ser entre 1 y 5 jugadores "))
    
    # Verifica que la cantidad de jugadores esté en el rango permitido
    while cantidadJugadores < 1 or cantidadJugadores > 5:
        cantidadJugadores = int(input("El numero ingresado no es valido, recuerda que la cantidad de jugadores debe ser entre 1 y 5. \nIntenta nuevamente: "))
...
while jugar:
    ...
    while partida:
        ...
        if jugadores[turno]["completado"] == False:
            ...
            if jugadores[turno].get("letrasCompletadas")[jugadores[turno].get("posicion")] == False:

                # Mensaje de turno para el jugador actual
                print("----------------------------------------------------------")
                print(f"Turno de {jugadores[turno].get('nombre')}. Recuerda que si no quieres arriesgar y prefieres pasar tu turno, debes enviar el mensaje sin nada escrito")

                # Solicita la respuesta del jugador
                respuesta = input(f"{jugadores[turno].get('rosco')[jugadores[turno].get('posicion')][1]}: ")
                ...
                if respuesta == jugadores[turno].get("rosco")[jugadores[turno].get("posicion")][0]:
                    # Respuesta correcta
                    print("La respuesta es Correcta!!!")

                    # Actualiza la cantidad de aciertos del jugador
                    jugadores[turno].update({"aciertos": jugadores[turno].get("aciertos") + 1})

                    # Actualiza la lista de letras completadas
                    aux = jugadores[turno].get("letrasCompletadas")
                    aux[jugadores[turno].get("posicion")] = True  # Marca la letra como completada
                    jugadores[turno].update({"letrasCompletadas": aux})

                    # Actualiza la posición del jugador
                    if False in aux:  # Si aún hay letras por completar
                        while aux[jugadores[turno].get("posicion")] == True:  # Busca la siguiente letra no completada
                            if jugadores[turno].get("posicion") == len(jugadores[turno].get("rosco")) - 1:
                                jugadores[turno].update({"posicion": 0})  # Vuelve al inicio si está en la última letra
                            else:
                                jugadores[turno].update({"posicion": jugadores[turno].get("posicion") + 1})  # Avanza a la siguiente letra
                ...
                # Verifica si el jugador eligió pasar su turno
                    ...
                    # Asegúrate de que la nueva posición no esté completada
                    while aux[jugadores[turno].get("posicion")] == True:
                        if jugadores[turno].get("posicion") == (len(jugadores[turno].get("rosco")) - 1):
                            jugadores[turno].update({"posicion": 0})  # Vuelve al inicio si está en la última letra
                        else:
                            jugadores[turno].update({"posicion": jugadores[turno].get("posicion") + 1})  # Avanza a la siguiente letra
                    ...
                    jugadores[turno].update({"errores": jugadores[turno].get("errores") + 1})
                ...
                jugadores[turno].update({"posicion": jugadores[turno].get("posicion") + 1})

        # Verifica si el jugador ha completado todas sus letras
        if False not in jugadores[turno].get("letrasCompletadas"):
        ...
        for jugador in jugadores:
            if jugador.get("completado") == False:
        ...
        if aux == 0:
            ...
            jugadoresOrdenados = sorted(jugadores, key=lambda x: x["aciertos"], reverse=True)
            # Mostrar la lista ordenada de puntajes
            for i in range(len(jugadoresOrdenados)):
                print(f"{i + 1} - {jugadoresOrdenados[i].get('nombre')} - Aciertos: {jugadoresOrdenados[i].get('aciertos')} - Errores: {jugadoresOrdenados[i].get('errores')} ")
                print("----------------------------------------")
